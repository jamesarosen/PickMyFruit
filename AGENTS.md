...
- **pnpm version**: pinned in root `package.json` `packageManager` (currently `pnpm@11.2.2`). CI and Docker read it via `pnpm/action-setup` and Corepack; `bin/session-start.sh` activates the same version when pnpm is missing from PATH. Workspace policy lives in `pnpm-workspace.yaml` (`minimumReleaseAge`, `blockExoticSubdeps`, `allowBuilds`, `supportedArchitectures`). The root `deploy` script (`fly deploy`) shadows pnpm's built-in `deploy` subcommand — use `pnpm pm deploy` for workspace package deployment (e.g. Dockerfile resend-sync step).
